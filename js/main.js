  ...
  if(this.status==200)
  {
    try { //costruire la tabella
      ...
      for (let index = 0; index < json.count; index++) {
        row += '< class="prenotazione" onclick="RedirectPrenotazione('+json.data[index].id+'">'; //CLICCANDO IN QUELLA PRENOTAZIONE RICHIEDE TRAMITE L'ID LA PRNOTAZIONE E STAMPA UNA PAGINA
         ...
         row += '<div class="canale">'+json.data[index].canale+'</div>';   //CANALE DELLA PRENOTAZIONE
         row += '<div class="stagione_periodo">'; // STAGIONE E PERIODO
           row += '<div class="stagione-group">'+stampaGiorno(json.data[index].dataPrenotazione)+'</div>';
           row += '<div class="periodo-group">' + stampaDataNoAnno(json.data[index].partenza) +' - '+ stampaDataNoAnno(json.data[index].arrivo) + '</div>';
         row += '</div>';
