...
function elencoPrenotazioni() {
    ...
    let xhrelenco = new XMLHttpRequest();
    xhrelenco.withCredentials = true;
    xhrelenco.open("GET", url, true);
    xhrelenco.send();
    xhrelenco.onload = handleElencoPrenotazioniResponse; 
  ...
  if(this.status==200)
  {
    try { //costruire la tabella
      ...
      for (let index = 0; index < json.count; index++) {
        row += '<div class="prenotazione" onclick="redirectPrenotazione('+json.data[index].id+'">'; //CLICCANDO IN QUELLA PRENOTAZIONE RICHIEDE TRAMITE L'ID LA PRNOTAZIONE E STAMPA UNA PAGINA
